...
function print_usage() {
     echo "CodecompCompose: A Compose Project for CodeComp"
     echo 'Usage:'
...
if [[ "$env" == "prod" ]]; then 
     if [[ -d "/etc/codecomp" && -d "/etc/codecomp/env" ]]; then 
          export COMPOSE_ENV_DIR="/etc/codecomp/env";
     elif [[ -d "${COMPOSE_DIR}/.env" ]]; then 
          export COMPOSE_ENV_DIR="${COMPOSE_DIR}/.env"
     else
          echo "No environment directory found at /etc/codecomp/env or ${COMPOSE_DIR}/.env for production.";
          echo "Quiting...";
          exit 1
     fi
fi
